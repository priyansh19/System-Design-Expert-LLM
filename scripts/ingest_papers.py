"""Ingest canonical system-design research papers into the grounding pool.

Downloads a curated set of foundational distributed-systems / architecture papers
(freely available PDFs), extracts and cleans their text, splits each into
word-bounded chunks, and *merges* them into data/generated/grounding.jsonl in the
same {source, heading, text, words} schema the pipeline already retrieves over.

These papers are the primary sources behind "best practices" and the "standard way
of designing systems": replicated storage, consensus, sharding, caching, stream
processing, observability, and the foundational principles.
...
Usage:
    python scripts/ingest_papers.py                 # download + ingest all
...
def main() -> None:
    ...
    for name, title, url in PAPERS:
        ...
        try:
            ...
            continue
        pieces = chunk(body, args.chunk_words, args.overlap)
        kept = 0
        for i, piece in enumerate(pieces):
            ...
            new_rows.append({"source": source, "heading": heading, "text": piece, "words": words})
            kept += 1
        print(f"  {name}: {kept} chunks ({len(body.split()):,} words)")
    ...
    total_words = sum(r["words"] for r in new_rows)
    print(
        f"\nWrote {len(new_rows)} paper chunks ({total_words:,} words) from {len(PAPERS)} papers.\n"
